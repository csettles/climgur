...
def upload(image_path, client_id):
    ...
    # I should do something with the exception. Later I will add logging for
    # these
    except Exception:
    ...
    data = {'image': b64image, 'title': '%s' %
            (os.path.basename(image_path))}  # make post data
    try:
        ...
        # print the stacktrace. Later, I should figure out what kind of
        # exceptions happen and handle them and log them
        sys.exit(str(e))
...
def main(args):
    # some of the logic in here is a bit backwards. I should probably fix this
    # to make it more nice
    if len(args) != 2:  # only 2 args needed
        sys.exit("use: %s /path/to/image/file.ext" %
                 (args[0]))  # exit with usage
    else:
    ...
    if client_id_env_var in os.environ.keys():  # check if env var exist
        ...
        sys.exit(
            "{!} Set environmental variable IMGUR_CLIENT_ID to your client_id :)")
